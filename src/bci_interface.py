...
- Gravação de dados atrelada ao paciente
"""
...
class SimpleCSVLogger:
    """Logger CSV simples para dados EEG"""
    ...
    def start_logging(self):
        ...
        with open(self.filename, 'w', newline='') as f:
            ...
            header = ['Timestamp'] + [f'EXG Channel {i}' for i in range(16)]
            writer.writerow(header)
        print(f"Gravação iniciada: {self.filename}")
        
    def log_data(self, eeg_data):
        """Adiciona dados ao log"""
        if self.is_logging:
            with self.lock:
                timestamp = datetime.now().isoformat()
                row = [timestamp] + list(eeg_data)
...
import os
import csv
sys.path.append(os.path.join(os.path.dirname(__file__), 'final'))
...
class DatabaseManager:
    ...
    def __init__(self, db_path="bci_patients.db"):
...
class StreamingWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        recording_layout.addWidget(QLabel("Paciente:"))
        recording_layout.addWidget(self.patient_combo)
        recording_layout.addWidget(self.refresh_patients_btn)
    ...
    def toggle_recording(self):
        ...
        if not self.is_recording:
            ...
            try:
                # Usar nosso logger simples
                self.csv_logger = SimpleCSVLogger(filename)
